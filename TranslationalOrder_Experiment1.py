...
def FTImage(rec_data,num_peaks):
	...
	if noFTImage:
		createRecDataImage(filename,rec_data,num_peaks,FTImgWidth,FTImgHeight)
...
def findReciprocalLatticeVectors(peaks,num_peaks,latticeSpacing):
	...
	while findingVectorsG:
		...
		if not inverseVectorsG:
			vectorsG.append([(8*pi*(endPoint[0]-float(FTImgWidth)/2)/(float(FTImgWidth)*latticeSpacing),8*pi*(endPoint[1]-float(FTImgHeight)/2)/(float(FTImgHeight)*latticeSpacing))])
			vectorsG.append([(8*pi*((endPoint[0]+2)-float(FTImgWidth)/2)/(float(FTImgWidth)*latticeSpacing),8*pi*(endPoint[1]-float(FTImgHeight)/2)/(float(FTImgHeight)*latticeSpacing))])
			vectorsG.append([(8*pi*((endPoint[0]+4)-float(FTImgWidth)/2)/(float(FTImgWidth)*latticeSpacing),8*pi*(endPoint[1]-float(FTImgHeight)/2)/(float(FTImgHeight)*latticeSpacing))])
			pts = [Point(int(endPoint[0]),int(endPoint[1]))]
			pts.append(Point(int(endPoint[0])+2,int(endPoint[1])))
			pts.append(Point(int(endPoint[0])+4,int(endPoint[1])))
			for pt in pts:
				mark = Circle(pt,3)
				ln = Line(center,pt)
				mark.setFill(color_rgb(255,0,0))
			...
			vectorsG[-3].append((8*pi*(endPoint[0]-float(FTImgWidth)/2)/(float(FTImgWidth)*latticeSpacing),8*pi*(endPoint[1]-float(FTImgHeight)/2)/(float(FTImgHeight)*latticeSpacing)))
			vectorsG[-2].append((8*pi*((endPoint[0]-2)-float(FTImgWidth)/2)/(float(FTImgWidth)*latticeSpacing),8*pi*(endPoint[1]-float(FTImgHeight)/2)/(float(FTImgHeight)*latticeSpacing)))
			vectorsG[-1].append((8*pi*((endPoint[0]-4)-float(FTImgWidth)/2)/(float(FTImgWidth)*latticeSpacing),8*pi*(endPoint[1]-float(FTImgHeight)/2)/(float(FTImgHeight)*latticeSpacing)))
			pts = [Point(int(endPoint[0]),int(endPoint[1]))]
			pts.append(Point(int(endPoint[0])-2,int(endPoint[1])))
			pts.append(Point(int(endPoint[0])-4,int(endPoint[1])))
			for pt in pts:
